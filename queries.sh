...
echo "$($PSQL "SELECT name FROM games FULL JOIN teams ON (games.winner_id = teams.team_id OR games.opponent_id = teams.team_id) WHERE year = 2014 AND round LIKE 'Eighth-Final' ORDER BY name")"
...
echo  "$($PSQL "SELECT year, name FROM teams FULL JOIN games ON winner_id = team_id WHERE round LIKE 'Final' ORDER BY year")"
